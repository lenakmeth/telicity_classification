...
def get_npz_data(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            l = line.strip().split('\t')
            temp_texts.append(l[0])
            temp_classes.append(int(l[-1]))
            
   # data = np.load(path_npz)
   # texts = data["texts"]
   # classes = data["classes"].astype(np.int8)
    texts = temp_texts
    classes = np.array(temp_classes, dtype=np.int8)
...
if __name__ == "__main__":
    ...
    texts, classes, len_rows = get_npz_data(sys.argv[1])
    ...
    texts_test, classes_test, len_rows_test = get_npz_data(sys.argv[3])
...
#     print('\nAccuracy in Percentage is =' + str(Accuracy * 100))
    
#     precision = precision_score(classes_test, Y_pred)
#     print('\nPrecision in Percentage is =' + str(precision * 100))
    
#     recall = recall_score(classes_test, Y_pred)
#     print('\nRecall in Percentage is =' + str(recall * 100))
    
#     f1 = f1_score(classes_test, Y_pred)
#     print('\nF1 Score in Percentage is =' + str(f1 * 100))
